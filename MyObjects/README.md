...
サンプル効率が悪いことへの対応として、開発の生産性を上げる下記の取り組みが考えられる    
1. テスト可能なモジュールに切り分ける  
...
## Day6  
### 強化学習 Tips  
1. サンプル効率が悪い点（学習に多くのデータが必要）への対応  
...
<img src="https://latex.codecogs.com/gif.latex?\pi^{n}=\left&space;(&space;1-a&space;\right&space;)^{n}\pi^{*}&plus;\alpha&space;\sum_{j=1}^{n}\left&space;(&space;1-a&space;\right&space;)^{j-1}\widehat{\pi}^{*j}" title="\pi^{n}=\left ( 1-a \right )^{n}\pi^{*}+\alpha \sum_{j=1}^{n}\left ( 1-a \right )^{j-1}\widehat{\pi}^{*j}" /></a>  
   ...
   1. 逆強化学習
   エキスパートの行動の背景にある報酬関数を推定する 
   エキスパートの行動を実現するように、戦略と報酬関数を交互に更新していく  
      * 線形計画法  
      エキスパートの戦略から得られる報酬が最大になるように線形の報酬関数を推定  
      * AIRL  
      エキスパートの状態遷移から得られる報酬が最大になるように線形の報酬関数を推定  
      * Max Entropy  
      エキスパートの状態遷移に近くなるように報酬パラメータを調整（報酬関数は非線形も可能）  
      * Bayesian  
      エキスパートの状態遷移確率と報酬実現確率の積が最大になるように、報酬の分布（パラメータ）を推定
      
## Day7  
### 強化学習 Tips  
強化学習の活用には、学習結果を活用する行動の最適化と、学習方法を活用する学習の最適化がある  
1. 行動の最適化  
ロボットの制御 ← カリキュラムラーニング、模倣学習    
対話への適用（広告配信、レコメンド） ← Contextual Bandit
2. 学習の最適化  
何らかの評価指標を報酬としてパラメータを学習させる  
ニューラルモデルのハイパーパラメータ、構造を強化学習で探索する方法もある（⇒GoogleのAutoML）  
